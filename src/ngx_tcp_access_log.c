...
static void ngx_tcp_log_write(ngx_tcp_session_t *s, ngx_tcp_log_t *log, u_char *buf, size_t len);


ngx_int_t 
ngx_tcp_access_log_handler(ngx_tcp_session_t *s)
{
	u_char					*line, *p;
	size_t					len;
	ngx_uint_t				l;
	...
	ngx_log_debug0(NGX_LOG_DEBUG_HTTP, s->connection->log, 0, "tcp access log handler");
	...
	for (l = 0; l < lscf->logs->nelts; ++l) {
		...
		if (ngx_time() == log[l].disk_full_time) {
			...
			/*
			 * on FressBSD writing to full filesystem with enabled softupdates
		...
		if (file && file->data) {
			...
			if (len > (size_t) (buffer->last - buffer->pos)) {
				...
				ngx_tcp_log_write(s, &log[l], buffer->start, buffer->pos - buffer->start);
		...
		if (file && file->buffer) {
			...
			if (len > (size_t) (file->last - file->pos)) {
				...
				ngx_tcp_log_write(s, &log[l], file->buffer, file->pos - file->buffer);
...
static void 
ngx_tcp_log_write(ngx_tcp_session_t *s, ngx_tcp_log_t *log, u_char *buf, size_t len)
{
	u_char		*name;
	time_t 		now;
	ssize_t		n;
